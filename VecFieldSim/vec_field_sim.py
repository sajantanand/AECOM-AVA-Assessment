...
# Example code to load numpy arrays
#empty = np.load('empty_new.npy')
#full = np.load('full_new.npy')
#base = np.load('base_new.npy')
#proposed = np.load('prop_new.npy')
...
class VecFieldSim:
    ...
    def __read_curv(dat):
        ...
        for _, sub_dat in enumerate(dat):
            length = len(sub_dat)
            # Only concerns 80% of wind field (with reptition)
    ...
    def __read_vel(dat):
        ...
        for _, sub_dat in enumerate(dat):
            length = len(sub_dat)
            # Concerns 100% of wind field (with reptition)
    ...
    def __calc_hist(self, mode=2, show_bar=True):
        ...
        if mode == 0:
            ...
            for _, dat in enumerate(self.arr_vel_dist):
                # Select 10th ~ 90th percentile of data (velocity dot product)
            ...
            for _, hist in enumerate(self.hist_curv_dist):
                metric = np.append(metric, [
                    [cv2.compareHist(self.hist_curv_dist[0][0], hist[0], cv2.HISTCMP_CHISQR),
                     # div 4 because cv2.HISTCMP_CHISQR_ALT
                     # has a different scalar multiple (2 instead of 1/2)
